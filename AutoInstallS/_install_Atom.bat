...
set PATH_PROGRAMS=\\172.2.2.4\Software\ProgramS\Atom
set PATH_LOGS=\\172.2.2.4\Software\ProgramS\_LogS

REM pushd %_CUR_PATH%

XCopy /y /c /h /r /i "%PATH_PROGRAMS%\AtomSetup.exe" "%ProgramFiles(x86)%\Atom\" && ECHO %ComputerName% ; Atom ; Copy ; OK ; %date% ; %time% >> "%PATH_LOGS%\%~n0.log" || ECHO %ComputerName% ; Atom ; Copy ; FAIL ; %date% ; %time% >> "%PATH_LOGS%\%~n0.log"
