set PATH_PROGRAMS=\\172.2.2.4\Software\ProgramS\SmartSuite20
...
:UNSSMRTBRD
REM IF EXIST "%CommonProgramFiles%\SMART Technologies\SMART Uninstaller\SMART Uninstaller.exe" copy /Y "%CommonProgramFiles%\SMART Technologies\SMART Uninstaller\SMART Uninstaller.exe" "%TEMP%"
REM IF EXIST "%CommonProgramFiles(x86)%\SMART Technologies\SMART Uninstaller\SMART Uninstaller.exe" copy /Y "%CommonProgramFiles(x86)%\SMART Technologies\SMART Uninstaller\SMART Uninstaller.exe" "%TEMP%"
REM IF EXIST "%TEMP%\SMART Uninstaller.exe" start /wait "SMART Uninstaller" "%TEMP%\SMART Uninstaller.exe" --noui --uninstall-all --uilevel none
REM IF EXIST "%TEMP%\SMART Uninstaller.exe" del /Q "%TEMP%\SMART Uninstaller.exe"
REM wmic product where name="SMART Install Manager" call uninstall

REM "%WinDir%\System32\msiexec.exe" /x "%PATH_PROGRAMS%\SMARTLearningSuite.msi" /qn /norestart && ECHO %ComputerName% ; SmartSuite ; uninstall ; STARTED ; %date% ; %time% >> "%PATH_LOGS%\%~n0.log"
REM IF NOT EXIST "%ProgramFiles(x86)%\SMART Technologies\Education Software\Notebook.exe" (ECHO %ComputerName% ; SmartSuite ; uninstall ; OK ; %date% ; %time% >> "%PATH_LOGS%\%~n0.log") ELSE (ECHO %ComputerName% ; SmartSuite ; uninstall ; FAIL ; %date% ; %time% >> "%PATH_LOGS%\%~n0.log")
